...
public class SpectatorCommand extends BaseCommand {
    @Command(name = "spectator",permission = "manhunt.command.spectator",usage = "<add/list> [player]")
    public CommandResult execute(Sender sender, String[] args) {
        if (args.length == 1 && args[0].equalsIgnoreCase("list")){
            ...
            PlayerManager.getPlayers().forEach(((uuid, role) -> {
                ...
                if (player != null){
                    if (role != Role.SPECTATOR)
                        return;
                    sender.sendMessage(CC.AQUA + player.getName() + CC.GREEN + " | Spectator");
                }
            }));
            sender.sendMessage(CC.SEPARATOR);
            ...
            try{
                PlayerManager.setPlayerRole(target,Role.SPECTATOR);
