...
public class RunnerCommand extends BaseCommand {
    @Command(name = "runner",permission = "manhunt.command.runner",usage = "<add/list> [player]")
        ...
        if (args.length == 1 && args[0].equalsIgnoreCase("list")){
            ...
            PlayerManager.getPlayers().forEach(((uuid, role) -> {
                ...
                if (player != null){
                    if (role != Role.RUNNER)
                        return;
                    sender.sendMessage(CC.AQUA + player.getName() + CC.GREEN + " | Runner");
                }
            }));
            sender.sendMessage(CC.SEPARATOR);
            ...
            try{
                PlayerManager.setPlayerRole(target,Role.RUNNER);
